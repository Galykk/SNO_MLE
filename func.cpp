...
void datahist(){
  ...
  //Make TH3F
    TH3F* data_hist = new TH3F("3D Data", "3D Data", 42, 0, 42, 26, 0, 26, 40, 0, 40);
    //Tree->("zaxis:yaxis:xaxis>>histname"), zaxis,yaxis,yaxis should be the variable name 
    //from the TTree object that you want on the respective axis of the TH3F
    tree->Draw("cthsun:radius:energy>>3D Data");

  // Write it to a file
    TFile* d2o_hist = new TFile("d2o_pdf.root", "RECREATE");   
    data_hist->Write();
    d2o_hist->Write();
}
...
//Write Bin Events to a Text File
void data_events(const char* name ,TH3F* h3){
 ofstream outfile(name);
 Double_t xbin;
 Double_t ybin;
 Double_t zbin;

//Loops through every possible bin to get the number of events in each of them
 for(int x=1;x<=h3->GetNbinsX();x++){
  for(int y=1;y<=h3->GetNbinsY();y++){
   for(int z=1;z<=h3->GetNbinsZ();z++){
    outfile << h3->GetBinContent(x,y,z) <<endl;
  }
  }
  }
}
...
Double_t jointPDF(const char* name, TH1F* ehist, TH1F* rhist, TH1F* chist){
    ofstream outfile(name);
    Double_t ebin_val;
    Double_t rbin_val;
    Double_t cbin_val;
...
void condPDF(const char* name, Double_t epsilon, int size, Double_t sum_of_bins){
...
Double_t logsum(Double_t *par){
  ifstream file0,file1,file2,file3,file4;
    ...
    while(file0 >> prob0){
       ...
       log_l+= data_entries*log(par[0]*prob0 + par[1]*prob1 + par[2]*prob2 + par[3]*prob3);
}
